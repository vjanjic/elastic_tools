...
template<typename L, typename R>
  inline __host__  L divide_ri(const L x, const R y)
{
    return (x + (y - 1)) / y;
}
...
template<typename L, typename R>
  inline __host__  L divide_rz(const L x, const R y)
{
    return x / y;
}
...
template<typename L, typename R>
  inline __host__  L round_i(const L x, const R y){ return y * divide_ri(x, y); }

// round x towards zero to the next multiple of y
template<typename L, typename R>
  inline __host__  L round_z(const L x, const R y){ return y * divide_rz(x, y); }
                                            ...
                                            size_t dynamic_smem_bytes)
{
 ...
 // printf("Limit threads %d\n", ctaLimitThreads);
  //printf("Limit blocks %d\n", ctaLimitBlocks);
