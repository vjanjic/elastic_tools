...
extern "C" void startSPKernel(size_t threads, size_t blocks, float *d_C, float *d_A, float *d_B, int vectorN, int elementN, cudaStream_t& stream) {
    scalarProdGPU<<<blocks, threads,0,stream>>>(d_C, d_A, d_B, vectorN, elementN);

}


cudaFuncAttributes getSPKernelProperties() {

		cudaFuncAttributes attributes;
		cudaFuncGetAttributes(&attributes, scalarProdGPU);
		return attributes;
